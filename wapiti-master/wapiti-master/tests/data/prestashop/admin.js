...
{
	const link = $('#link_rewrite_' + id_language);
	if (link[0])
	{
...
function checkAll(pForm)
{
	...
	{
		const objName = pForm.elements[i].name;
		const objType = pForm.elements[i].type;
...
let tpl_viewing_window = null;
function viewTemplates(id_select, prefix, ext)
{
	const loc = $(id_select).val();
	if (loc != 0)
	{
...
{
	const $zone = $('#txt_msg');
	const sl_value = sl.options[sl.selectedIndex].value;
	...
	view.bind("scroll resize", function(e)
	{
		message.each(function(el){
			if (message.length)
			{
				...
				const viewTop = view.scrollTop() + 15;
	...
	$('.action_tab_module').each( function (){
		$(this).click(function () {
			...
			if ($(option).data('onclick') != '')
			{
				...
				const f = eval("(function(){ "+$(option).data('onclick')+"})");
				if (f.call())
...
{
	const id = action+'_'+row_id;
	const current_element = $('#details_'+id);
			...
			success: function(data) {
				if (typeof(data.use_parent_structure) == 'undefined' || (data.use_parent_structure == true))
				{
					...
					$.each(data.data, function(it, row)
					{
						...
						const content = $('<tr class="action_details details_'+id+' '+(alt_row ? 'alt_row' : '')+'"></tr>');
						content.append($('<td class="empty"></td>'));
						let first = true;
						let count = 0; // Number of non-empty collum
...
function hideOtherLanguage(id)
{
	...
	const id_old_language = id_language;
	id_language = id;
...
function check_for_all_accesses(tabsize, tabnumber)
{
	...
	let res = 0;
	let right = 0;
	const rights = ['view', 'add', 'edit', 'delete', 'all']; 
...
function saveCustomerNote(customerId){
	...
	const data = 'token=' + token_admin_customers + '&tab=AdminCustomers&ajax=1&action=updateCustomerNote&id_customer=' + customerId + '&note=' + encodeURIComponent(noteContent);
	$.ajax({
		type: "POST",
		url: "index.php",
		data: data,
