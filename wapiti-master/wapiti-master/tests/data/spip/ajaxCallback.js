...
if(!jQuery.load_handlers) {
    ...
    for ( let i = 0; i < jQuery.load_handlers.length; i++ )
	...
	jQuery.fn.load = function( url, params, callback ) {
		...
		const callback2 = function(res,status) {triggerAjaxLoad(this);callback(res,status);};
	...
	jQuery.ajax = function(type) {
		...
		if (jQuery.ajax.caller==jQuery.fn._load) return jQuery._ACBajax( type);
			const orig_complete = type.complete || function() {};
			type.complete = function(res,status) {
				// Do not fire OnAjaxLoad if the dataType is not html
				const dataType = type.dataType;
				const ct = (res && (typeof res.getResponseHeader == 'function'))
					? res.getResponseHeader("content-type"): '';
				const xml = !dataType && ct && ct.indexOf("xml") >= 0;
...
jQuery.fn.animeajax = function(end) {
	...
	if (typeof ajax_image_searching != 'undefined'){
		const i = (this).find('.image_loading');
...
jQuery.fn.positionner = function() {
	const offset = jQuery(this).offset({'scroll':false});
	const hauteur = parseInt(jQuery(this).css('height'));
...
const virtualbuffer_id='spip_virtualbufferupdate';
function initReaderBuffer(){
	if (jQuery('#'+virtualbuffer_id).length) return;
	jQuery('body').append('<p style="float:left;width:0;height:0;position:absolute;left:-5000;top:-5000;"><input type="hidden" name="'+virtualbuffer_id+'" id="'+virtualbuffer_id+'" value="0" /></p>');
}
function updateReaderBuffer(){
	const i = jQuery('#'+virtualbuffer_id);
	if (!i.length) return;
...
let ajax_confirm=true;
let ajax_confirm_date=0;
const spip_confirm = window.confirm;
...
let ajaxbloc_selecteur;
jQuery.fn.ajaxbloc = function() {
	if (this.length)
		initReaderBuffer();
  return this.each(function() {
  jQuery('div.ajaxbloc',this).ajaxbloc(); // traiter les enfants d'abord
	const blocfrag = jQuery(this);
	
	const on_pagination = function(c) {
		jQuery(blocfrag)
		.html(c)
		.removeClass('loading')
		.positionner();
	...
	let ajax_env = (""+blocfrag.attr('class')).match(/env-([^ ]+)/);
