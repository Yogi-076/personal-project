...
export function renderTab(state: AppViewState, tab: Tab) {
  ...
  const name = isChat && state.assistantName && state.assistantName !== "Assistant" ? state.assistantName : titleForTab(tab);
  const avatarUrl = isChat ? state.chatAvatarUrl : null;
  ...
  return html`
    ...
    >
      <span class="nav-item__icon" aria-hidden="true">
        ${isChat && avatarUrl
      ? html`<img src="${avatarUrl}" style="width: 18px; height: 18px; border-radius: 50%; object-fit: contain; background: #000;" />`
